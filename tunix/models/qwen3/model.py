...
class Einsum(nnx.Module):
  ...
  def __init__(
    ...
    self.shape = shape
    self.w = nnx.Param(
        nnx.initializers.normal(dtype=dtype)(rngs.params(), shape),
        sharding=sharding,
    )
    self.dtype = dtype
  ...
  def __call__(self, x: jaxtyping.ArrayLike) -> jaxtyping.Array:
    # w = self.w.value.astype(self.dtype)
...
class Embedder(nnx.Module):
  ...
  def __init__(
    ...
    self.input_embedding = nnx.Param(
        nnx.initializers.normal(dtype=dtype)(
  ...
  def encode(self, x: jaxtyping.ArrayLike) -> jaxtyping.Array:
    # W = jnp.asarray(self.input_embedding.value, self.dtype)
    # idx = jnp.asarray(x, jnp.int32)
    # emb = W[idx]
    # return shard(emb, self.shd_config.act_btd)
...
class RMSNorm(nnx.Module):
  ...
  def __call__(self, x: jaxtyping.Array) -> jaxtyping.Array:
    dtype = x.dtype
    x32 = jnp.asarray(x, self.dtype)
    rms = jnp.sqrt(jnp.mean(x32 * x32, axis=-1, keepdims=True) + self.norm_eps)
    y32 = (x32 / rms) * jnp.asarray(self.w.value, self.dtype)
    return jnp.asarray(y32, dtype)
