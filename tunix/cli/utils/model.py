...
def apply_lora_to_model(base_model, mesh, lora_config, pad_id):
  ...
  sharded = shard_input(model_input)
  with mesh:
    _ = base_model(**sharded)
...
def perturb(model, pad_id):
  import functools
  my_gen_model_input_fn = functools.partial(gen_model_input_fn_ift, pad_id=pad_id, is_causal=True)
  train_example = {
    "input_tokens": jnp.ones((32, 512)).astype(jnp.int32),
    "input_mask": jnp.ones((32, 512)) == 1
  }
  train_example = my_gen_model_input_fn(train_example)
  _ = model(train_example["input_tokens"], train_example["positions"], None, train_example["attention_mask"])
  jax.debug.print("##############")
  jax.debug.print("##############")
  jax.debug.print("##############")

  jax.debug.print("PERT SHAPE {}", model.xgrad1.shape)
  jax.debug.print("##############")
  jax.debug.print("##############")
  jax.debug.print("##############")
  jax.debug.print("##############")
...
def gen_model_input_fn_ift(x, pad_id, is_causal):
...
def create_model(
  ...
  tokenizer = create_tokenizer(tokenizer_config, tokenizer_path)
  pad_id = tokenizer.pad_id()
  if model_config.get('lora_config'):
    # Apply Lora to model if given lora config
    model = apply_lora_to_model(model, mesh, model_config['lora_config'], pad_id)
  else:
